...
export default function Nationality({isEdit, setIsEdit}) {
    ...
    const onFinish = useCallback(async (values) => {
        try {
            ...
            const res = await userApi.update({nationality}, user.id);
        ...
        setLoading(false);
        setActive(!active);
    }, [user, active, dispatch, nationality])
        ...
        <div className="py-2 px-6 flex">
            ...
            <div className="flex flex-1 justify-between">
                <div className="w-3/4">
                    {active &&
                        <div>
                            <Typography.Text>{user.nationality ? getContry(user.nationality).name : 'Chọn vùng/quốc gia của bạn'}</Typography.Text>
                        </div>
                    }
                    ...
                    {!active &&
                        ...
                        >
                            <Row gutter={24} >
                                <Col span={12}>
                                    <Form.Item label="Quốc tịch" name="nationality" rules={[{ required: true, message: 'Vui lòng chọn vùng/quốc gia của bạn' }]}>
                                        <Select
                                            showSearch
